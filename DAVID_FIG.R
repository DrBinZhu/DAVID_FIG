...
pdf("enrichment_KEGG.pdf", width=4.8, height=1.3)
print(p+q)
dev.off()
...
choosedata <- data[,1]=='GOTERM_BP_DIRECT'
data <- data[data[,1]=='GOTERM_BP_DIRECT',]
data <- unique(data)
...
ggplot(data3, aes(x=pvalue, y= reorder(Pathway,pvalue), size = Number))+
  geom_point()+
  labs(x = "-Log10(pvalue)", y = "GOTERM_BP_DIRECT", size = "Number of genes")
...
pdf("enrichment_GOTERM_BP_DIRECT.pdf", width=7, height=2.5)
print(p+q)
dev.off()
...
choosedata <- data[,1]=='UP_KW_BIOLOGICAL_PROCESS'
data <- data[data[,1]=='UP_KW_BIOLOGICAL_PROCESS',]
data <- unique(data)
...
ggplot(data3, aes(x=pvalue, y= reorder(Pathway,pvalue), size = Number))+
  geom_point()+
  labs(x = "-Log10(pvalue)", y = "UP_KW_BIOLOGICAL_PROCESS", size = "Number of genes")
...
pdf("enrichment_UP_KW_BIOLOGICAL_PROCESS.pdf", width=4.8, height=1.3)
print(p+q)
dev.off()
